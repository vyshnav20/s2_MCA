...
SELECT s.Course_number,s.Semester,s.Year,count(g.Student_number) as No_of_student from section s  join grade_report g on s.Section_identifier = g.Section_identifier where s.Instructor="King" GROUP BY s.Course_number, s.Semester, s.Year;
